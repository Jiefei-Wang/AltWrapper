...
We will illustrate the package with a simplest example. Suppose we want to represent a sequence vector in a compact format, which means the sequence will only have a starting value and its increment as the data. We will use this compacted sequence to create a standard R vecotr by defining `ALTREP` APIS.
