...
ALTREP is still underdevelopment and some R functions do not catch up with it. At the time of this writting, only R 3.7 devel version can correctly print the object. Before R 3.7, the variable can be printed out either by using `[` operator to force R to use `get_Elt` method, or by using `printAltWrapper` function providing  by AltWrapper package.
```{r}
## Use [ operator
A_vector[1:10]
